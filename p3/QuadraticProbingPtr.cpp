        ...
        {
                // Insert x as active
            const HashedObj *value = new HashedObj;
	    value = &x;
            int currentPos = findPos( x );
            if( isActive( currentPos ) )
                return;
            array[ currentPos ] = value;//HashEntry( x, ACTIVE );
...
/* 7*/      return currentPos;
        ...
        void QuadraticPtrHashTable<HashedObj>::remove( const HashedObj & x )
        {
            int currentPos = findPos( x );
            if( isActive( currentPos ) )
                array[ currentPos ] = &ITEM_NOT_FOUND; //deleted
        }
        ...
        {
        	if(array[currentPos] && *array[currentPos] != ITEM_NOT_FOUND) 
	   		return true;
		else
			return false;
        }
